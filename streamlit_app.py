...
def build_retrieval_query(query, messages, max_turns=MEMORY_TURNS):
    history = build_history_text(messages, max_turns=max_turns)
    if not history:
        return query
    return f"Conversation context:\n{history}\n\nCurrent user question:\n{query}"


def answer_from_docs(query, hits, chat_model, min_sim=MIN_SIM, history_text=""):
...
4) Use chat history only to resolve references like "that", "above", "it"; never treat history as factual source.
5) End with citations like (source.pdf, page X).
